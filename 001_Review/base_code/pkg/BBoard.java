...
public class BBoard {		// This is your main file that connects all classes.
    ...
    public void loadUsers(String inputFile) throws FileNotFoundException {
        ...
        while (sc.hasNext()) {
            ...
            PassHolder = sc.next();
            User user = new User(NameHolder, PassHolder);
            Userlist.add(user);
        }
        sc.close();
    }
	...
	// If not, it will keep asking until a match is found or the user tyq' or 'Q' as username to quit
	// When the users chooses to quit, sayu "Bye!" and return from the login function
	public void login() {
    Scanner sc = new Scanner(System.in);
    while (true) {
        System.out.println("Please enter your username ( or enter 'q' or 'Q' to quit )");
        ...
        String userinputuah = sc.nextLine();

        User matchedUser = null;
        for (User u : Userlist) {
            if (u.getUsername().equals(userinput) && u.check(userinput, userinputuah)) {
                matchedUser = u;
                break;
            }
        }
...
private void addReply() {
        ...
        String subject = "Re: " + parent.getSubject();
        // Assuming Reply(String auth, String subj, String bod, int id)
        Reply reply = new Reply(CurrentUser.getUsername(), subject, body, Messagelist.size() + 1);
...
private void setPassword() {
    ...
    while (true) {
        System.out.println("Enter your old password (c/C to cancel):");
